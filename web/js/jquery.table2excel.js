...
//(function ( $, window, document, undefined ) {
/*Уберём после, если исправленный себя нормально покажет
$(document).ready(function() {
    var pluginName = "table2excel",
...
function export_excel(class_name, filename)
{
	filename = filename || 'myFileName';
	class_name = class_name || 'table2excel';
	
	//Проверим таблицу на пустоту
  var count_rows = document.querySelectorAll('.'+class_name+' tbody tr').length;
  
  //console.info(count_rows);
  
	if( count_rows > 0 ){
		$("." + class_name).table2excel({
			exclude: ".noExl",
